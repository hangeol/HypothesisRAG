#!/usr/bin/env python3
"""
ARC Easy Evaluation for RAG Comparison Experiment (Async Parallel Version)

This script evaluates different RAG modes on ARC Easy testset
...
class ARCEasyDataset:
    ...
    def __init__(self, benchmark_path: Optional[str] = None):
        if benchmark_path is None:
            benchmark_path = "/root/HypothesisRAG/data/arc_easy_train.json"
        ...
        self.dataset = benchmark["arc_easy_train"]
        self.index = sorted(self.dataset.keys())
        print(f"✓ Loaded {len(self)} ARC Easy questions")
...
class AsyncRAGEvaluator:
    ...
    def generate_queries_grpo_rewriter(
        ...
        import torch
        from data.arc_easy_loader import format_rewriter_prompt
...
async def run_evaluation_async(
    ...
    print("=" * 80)
    print("ARC Easy Evaluation (MAX ASYNC)")
    print("=" * 80)
    ...
    if not model_suffix:
        model_suffix = "model"
    output_file = os.path.join(output_dir, f"arc_easy_{mode_suffix}_{model_suffix}_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="ARC Easy Max Async Parallel Evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # CoT baseline only (no RAG)
  python evaluate_arc_easy.py --modes cot --max-questions 1273

  # RAG modes only
  python evaluate_arc_easy.py --modes direct baseline planning --max-questions 1273

  # All modes including CoT
  python evaluate_arc_easy.py --modes cot direct baseline planning --max-questions 100

  # Quick test (10 questions)
  python evaluate_arc_easy.py --max-questions 10

  # Full RAG evaluation with max parallelism
  python evaluate_arc_easy.py --max-questions 1273 --max-concurrent 100
        """
    )
