...
import tiktoken
import sys
# Use relative imports to avoid conflicts
try:
    from .utils import RetrievalSystem, DocExtracter
    from .template import *
except ImportError:
    # Fallback to absolute imports if relative doesn't work
    sys.path.append("src")
    from utils import RetrievalSystem, DocExtracter
    from template import *
...
#cursoragent modify start
# Add vLLM import with fallback
try:
    from vllm import LLM, SamplingParams
    VLLM_AVAILABLE = True
except ImportError:
    VLLM_AVAILABLE = False
    print("Warning: vLLM not available. Using transformers pipeline instead.")
#cursoragent modify end
...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False, use_vllm=False):
        ...
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            if "mixtral" in llm_name.lower():
                ...
                self.max_length = 2048
                self.context_length = 1024
            elif "medgemma" in llm_name.lower():
                # MedGemma configuration - medical domain specialized model
                self.max_length = 16384  # MedGemma 4B-IT supports up to 8K context, increased for longer responses
                self.context_length = 7168
                # Use default chat template for Gemma models
            
            #cursoragent modify start
            # Initialize model based on user choice
            if self.use_vllm:
                print(f"Initializing vLLM for {self.llm_name}")
                self.model = LLM(
                    model=self.llm_name,
                    dtype="bfloat16",
                ...
                self.sampling_params = SamplingParams(
                    temperature=0.0,
                    max_tokens=4096,  # 더 긴 응답을 위해 4096으로 증가
                    stop=["###", "User:", "\n\n\n"]
                )
            else:
                print(f"Initializing transformers pipeline for {self.llm_name}")
    ...
    def generate(self, messages, **kwargs):
        ...
        if "openai" in self.llm_name.lower():
            ...
            #cursoragent modify start
            # Handle vLLM vs transformers generation
            if self.use_vllm:
                # Check if messages is a list of dicts (chat format) or a simple string
                ...
                # Handle response based on whether it's chat format or simple prompt
                if isinstance(messages, list) and len(messages) > 0 and isinstance(messages[0], dict):
                    # Chat format: remove the prompt part
                    ans = response[0]["generated_text"][len(prompt):]
                else:
                    # Simple prompt: return the full generated text
