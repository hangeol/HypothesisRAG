#!/usr/bin/env python3
"""
OpenBookQA Additional Evaluation for RAG Comparison Experiment (Async Parallel Version)

This script evaluates different RAG modes on OpenBookQA Additional testset
...
COT_SYSTEM_PROMPT = '''You are a helpful expert with broad general and commonsense knowledge, and your task is to answer a multi-choice question requiring multi-step reasoning and rich text comprehension. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''

# RAG (with documents)
MIRAGE_SYSTEM_PROMPT = '''You are a helpful expert with broad general and commonsense knowledge, and your task is to answer a multi-choice question requiring multi-step reasoning and rich text comprehension using the relevant documents. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''
...
PLANNING_PROMPT = """Analyze the following question requiring multi-step reasoning and rich text comprehension and extract key information.

Question: {question}

Extract:
1. observed_features: List 3-7 key concepts, findings, or conditions mentioned
...
# ============================================================================
PLANNING_V2_PROMPT = """You are a expert with broad general and commonsense knowledge. Analyze this reasoning question and provide a logical reasoning plan.
...
{{
    "key_reasoning_features": ["list 3-5 most important reasoning clues from the question"],
...
Query 1: Search for the PRIMARY hypothesis with key concepts
Query 2: Search for DIFFERENTIAL hypothesis comparison
Query 3: Search for DISTINGUISHING lab/imaging/factual findings
...
# ============================================================================
PLANNING_V3_PROMPT = """You are an expert commonsense reasoning expert. Analyze this reasoning question and create a focused reasoning plan.
...
{{
    ...
    "key_reasoning_clues": ["list 2-4 most critical factual findings"],
...
PLANNING_V3_QUERY_PROMPT = """You are a general search expert. Generate the MINIMUM number of highly specific search queries needed to answer this question.
...
# ============================================================================
PLANNING_V4_PROMPT = """You are an expert commonsense reasoning expert taking a open book exam.
...
Step 2: Make your BEST GUESS for the answer based on commonsense knowledge.
...
PLANNING_V4_QUERY_PROMPT = """Generate 3 highly targeted search queries to find evidence for this question requiring multi-step reasoning and rich text comprehension.
...
Query 3: Find specific factual/commonsense features
...
# ============================================================================
PLANNING_V6_PROMPT = """You are an expert commonsense reasoning expert. Analyze this question using alternative hypothesis approach.
...
{{
    "hypothesis_1": {{
        ...
        "supporting_features": ["2-3 reasoning features that support this"],
...
Query 4: Confirming underlying logic/mechanism
Query 5: Clinical presentation comparison
Query 6 (optional): Differences in underlying logic
...
# ============================================================================
# OpenBookQA Additional Dataset Loader
# ============================================================================
class OpenBookQAAdditionalDataset:
    """OpenBookQA Additional Dataset loader"""
    
    def __init__(self, benchmark_path: Optional[str] = None):
        if benchmark_path is None:
            benchmark_path = "/root/HypothesisRAG/data/openbookqa_additional_train.json"
        ...
        self.dataset = benchmark["openbookqa_additional_train"]
        self.index = sorted(self.dataset.keys())
        print(f"✓ Loaded {len(self)} OpenBookQA Additional questions")
...
class AsyncRAGEvaluator:
    ...
    async def generate_subqueries_async(
        ...
        if len(queries) < 3:
            ...
            queries = [
                question,
                f"{' '.join(words[:3])} concepts hypothesis" if words else question,
                f"{' '.join(words[:3])} treatment" if words else question,
                f"{' '.join(words[1:4])} medical" if len(words) > 1 else question,
                f"{' '.join(words[:2])} underlying logic" if len(words) > 1 else question,
            ][:self.num_subqueries]
        
        return queries
    
    # =========================================================================
    # Planning V2: Improved logical reasoning approach
    # =========================================================================
    async def generate_plan_v2_async(
        ...
        """Generate improved planning output with logical reasoning"""
        options_text = "\n".join([f"{k}. {v}" for k, v in sorted(options.items())])
        prompt = PLANNING_V2_PROMPT.format(question=question, options=options_text)
        
        messages = [
            {"role": "system", "content": "You are an expert commonsense reasoning expert. Provide precise, clinically-focused analysis."},
            {"role": "user", "content": prompt}
        ]
        ...
        if not plan.get("key_reasoning_features"):
            words = re.findall(r'\b[A-Za-z]{4,}\b', question)
            plan["key_reasoning_features"] = words[:5] if words else ["symptom"]
    ...
    async def generate_queries_from_plan_v2_async(
        ...
        """Generate targeted queries based on the reasoning plan"""
        
        key_features = ", ".join(plan.get("key_reasoning_features", [])[:5])
        ...
        messages = [
            {"role": "system", "content": "You are a general information retrieval expert. Generate precise, targeted search queries."},
            {"role": "user", "content": prompt}
        ]
        ...
        if len(queries) < 3:
            queries = [
                ...
                f"{distinguishing} reasoning criteria",
                f"{primary_hypothesis} underlying logic mechanism",
    ...
    async def generate_plan_v3_async(
        ...
        messages = [
            {"role": "system", "content": "You are an expert commonsense reasoning expert. Be concise and precise."},
            {"role": "user", "content": prompt}
        ]
        ...
        plan.setdefault("key_reasoning_clues", [])
        ...
        if not plan["key_reasoning_clues"]:
            words = re.findall(r'\b[A-Za-z]{4,}\b', question)
            plan["key_reasoning_clues"] = words[:4] if words else ["symptom"]
    ...
    async def generate_queries_from_plan_v3_async(
        ...
        key_clues = ", ".join(plan.get("key_reasoning_clues", [])[:4])
        ...
        messages = [
            {"role": "system", "content": "You are a general search expert. Generate only essential, highly targeted queries. Quality over quantity."},
            {"role": "user", "content": prompt}
        ]
    ...
    async def generate_plan_v4_async(
        ...
        messages = [
            {"role": "system", "content": "You are an expert commonsense reasoning expert. Make your best reasoned guess and identify what evidence would confirm it."},
            {"role": "user", "content": prompt}
        ]
    ...
    def generate_queries_grpo_rewriter(
        ...
        import torch
        from data.openbookqa_additional_loader import format_rewriter_prompt
    ...
    async def generate_plan_v6_async(
        ...
        messages = [
            {"role": "system", "content": "You are an expert commonsense reasoning expert. Use alternative hypotheses to generate two competing hypotheses."},
            {"role": "user", "content": prompt}
        ]
    ...
    async def generate_queries_from_plan_v6_async(
        ...
        if len(queries) < 3:
            queries = [
                ...
                f"factual presentation comparison",
            ]
...
async def run_evaluation_async(
    ...
    print("=" * 80)
    print("OpenBookQA Additional Evaluation (MAX ASYNC)")
    print("=" * 80)
    ...
    # Load dataset
    dataset = OpenBookQAAdditionalDataset()
    ...
    if not model_suffix:
        model_suffix = "model"
    output_file = os.path.join(output_dir, f"openbookqa_additional_{mode_suffix}_{model_suffix}_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="OpenBookQA Additional Max Async Parallel Evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # CoT baseline only (no RAG)
  python evaluate_openbookqa_additional.py --modes cot --max-questions 1273

  # RAG modes only
  python evaluate_openbookqa_additional.py --modes direct baseline planning --max-questions 1273

  # All modes including CoT
  python evaluate_openbookqa_additional.py --modes cot direct baseline planning --max-questions 100

  # Quick test (10 questions)
  python evaluate_openbookqa_additional.py --max-questions 10

  # Full RAG evaluation with max parallelism
  python evaluate_openbookqa_additional.py --max-questions 1273 --max-concurrent 100
        """
    )
