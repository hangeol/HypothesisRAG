#!/usr/bin/env python3
"""
WorldTree Evaluation for RAG Comparison Experiment (Async Parallel Version)

This script evaluates different RAG modes on WorldTree testset
...
COT_SYSTEM_PROMPT = '''You are a helpful expert in elementary science explanations, and your task is to answer a multi-choice elementary science question needing logical explanation. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''

# RAG (with documents)
MIRAGE_SYSTEM_PROMPT = '''You are a helpful expert in elementary science explanations, and your task is to answer a multi-choice elementary science question needing logical explanation using the relevant documents. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''
...
PLANNING_PROMPT = """Analyze the following elementary science question needing logical explanation and extract key information.
...
# ============================================================================
PLANNING_V2_PROMPT = """You are a expert in elementary science explanations. Analyze this science question and provide a explanation-driven reasoning plan.
...
{{
    "key_scientific_concepts": ["list 3-5 most important scientific clues from the question"],
...
Query 3: Search for DISTINGUISHING lab/imaging/scientific facts
...
# ============================================================================
PLANNING_V3_PROMPT = """You are an expert science explanation expert. Analyze this science question and create a focused reasoning plan.
...
{{
    ...
    "key_scientific_clues": ["list 2-4 most critical scientific facts"],
...
PLANNING_V3_QUERY_PROMPT = """You are a scientific explanation search expert. Generate the MINIMUM number of highly specific search queries needed to answer this question.
...
4. For complex alternative explanations, use 4-6 queries
...
# ============================================================================
PLANNING_V4_PROMPT = """You are an expert science explanation expert taking a science exam.
...
Step 2: Make your BEST GUESS for the answer based on domain and world knowledge.
...
PLANNING_V4_QUERY_PROMPT = """Generate 3 highly targeted search queries to find evidence for this elementary science question needing logical explanation.
...
Query 2: Find distinguishing criteria for alternative explanations  
...
# ============================================================================
PLANNING_V6_PROMPT = """You are an expert science explanation expert. Analyze this question using alternative explanation approach.
...
{{
    "hypothesis_1": {{
        ...
        "supporting_features": ["2-3 scientific concepts that support this"],
...
Query 4: Confirming underlying scientific principles/mechanism
Query 5: Clinical presentation comparison
Query 6 (optional): Differences in underlying explanation principles
...
# ============================================================================
# WorldTree Dataset Loader
# ============================================================================
class WorldTreeDataset:
    """WorldTree Dataset loader"""
    
    def __init__(self, benchmark_path: Optional[str] = None):
        if benchmark_path is None:
            benchmark_path = "/root/HypothesisRAG/data/worldtree_train.json"
        ...
        self.dataset = benchmark["worldtree_train"]
        self.index = sorted(self.dataset.keys())
        print(f"✓ Loaded {len(self)} WorldTree questions")
...
class AsyncRAGEvaluator:
    ...
    async def generate_subqueries_async(
        ...
        if len(queries) < 3:
            ...
            queries = [
                ...
                f"{' '.join(words[:2])} underlying scientific principles" if len(words) > 1 else question,
            ][:self.num_subqueries]
        
        return queries
    
    # =========================================================================
    # Planning V2: Improved explanation-driven reasoning approach
    # =========================================================================
    async def generate_plan_v2_async(
        ...
        """Generate improved planning output with explanation-driven reasoning"""
        options_text = "\n".join([f"{k}. {v}" for k, v in sorted(options.items())])
        prompt = PLANNING_V2_PROMPT.format(question=question, options=options_text)
        
        messages = [
            {"role": "system", "content": "You are an expert science explanation expert. Provide precise, clinically-focused analysis."},
            {"role": "user", "content": prompt}
        ]
        ...
        if not plan.get("key_scientific_concepts"):
            words = re.findall(r'\b[A-Za-z]{4,}\b', question)
            plan["key_scientific_concepts"] = words[:5] if words else ["symptom"]
    ...
    async def generate_queries_from_plan_v2_async(
        ...
        key_features = ", ".join(plan.get("key_scientific_concepts", [])[:5])
        ...
        messages = [
            {"role": "system", "content": "You are a scientific knowledge retrieval expert. Generate precise, targeted search queries."},
            {"role": "user", "content": prompt}
        ]
        ...
        if len(queries) < 3:
            queries = [
                ...
                f"{primary_hypothesis} vs {alternatives} alternative explanations",
                f"{distinguishing} knowledge criteria",
                f"{primary_hypothesis} underlying scientific principles mechanism",
    ...
    async def generate_plan_v3_async(
        ...
        messages = [
            {"role": "system", "content": "You are an expert science explanation expert. Be concise and precise."},
            {"role": "user", "content": prompt}
        ]
    ...
    async def generate_queries_from_plan_v3_async(
        ...
        messages = [
            {"role": "system", "content": "You are a scientific explanation search expert. Generate only essential, highly targeted queries. Quality over quantity."},
            {"role": "user", "content": prompt}
        ]
    ...
    async def generate_plan_v4_async(
        ...
        messages = [
            {"role": "system", "content": "You are an expert science explanation expert. Make your best reasoned guess and identify what evidence would confirm it."},
            {"role": "user", "content": prompt}
        ]
    ...
    def generate_queries_grpo_rewriter(
        ...
        import torch
        from data.worldtree_loader import format_rewriter_prompt
    ...
    async def generate_plan_v6_async(
        ...
        """Generate dual hypothesis plan for alternative explanations"""
        options_text = "\n".join([f"{k}. {v}" for k, v in sorted(options.items())])
        prompt = PLANNING_V6_PROMPT.format(question=question, options=options_text)
        
        messages = [
            {"role": "system", "content": "You are an expert science explanation expert. Use alternative explanations to generate two competing hypotheses."},
            {"role": "user", "content": prompt}
        ]
...
async def run_evaluation_async(
    ...
    print("=" * 80)
    print("WorldTree Evaluation (MAX ASYNC)")
    print("=" * 80)
    ...
    if not model_suffix:
        model_suffix = "model"
    output_file = os.path.join(output_dir, f"worldtree_{mode_suffix}_{model_suffix}_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="WorldTree Max Async Parallel Evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # CoT baseline only (no RAG)
  python evaluate_worldtree.py --modes cot --max-questions 1273

  # RAG modes only
  python evaluate_worldtree.py --modes direct baseline planning --max-questions 1273

  # All modes including CoT
  python evaluate_worldtree.py --modes cot direct baseline planning --max-questions 100

  # Quick test (10 questions)
  python evaluate_worldtree.py --max-questions 10

  # Full RAG evaluation with max parallelism
  python evaluate_worldtree.py --max-questions 1273 --max-concurrent 100
        """
    )
