#!/usr/bin/env python3
"""
ScienceQA Evaluation for RAG Comparison Experiment (Async Parallel Version)

This script evaluates different RAG modes on ScienceQA testset
...
COT_SYSTEM_PROMPT = '''You are a helpful expert in elementary and high school science (natural, language, and social science), and your task is to answer a multi-choice science question. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''

# RAG (with documents)
MIRAGE_SYSTEM_PROMPT = '''You are a helpful expert in elementary and high school science (natural, language, and social science), and your task is to answer a multi-choice science question using the relevant documents. Please first think step-by-step and then choose the answer from the provided options. Organize your output in a json formatted as Dict{"step_by_step_thinking": Str(explanation), "answer_choice": Str{A/B/C/...}}. Your responses will be used for research purposes only, so please have a definite answer.'''
...
# ============================================================================
PLANNING_V2_PROMPT = """You are a expert in elementary and high school science (natural, language, and social science). Analyze this science question and provide a scientific reasoning plan.
...
class ScienceQADataset:
    ...
    def __init__(self, benchmark_path: Optional[str] = None):
        if benchmark_path is None:
            benchmark_path = "/root/HypothesisRAG/data/scienceqa_train.json"
        ...
        self.dataset = benchmark["scienceqa_train"]
        self.index = sorted(self.dataset.keys())
        print(f"✓ Loaded {len(self)} ScienceQA questions")
...
class AsyncRAGEvaluator:
    ...
    def generate_queries_grpo_rewriter(
        ...
        import torch
        from data.scienceqa_loader import format_rewriter_prompt
...
async def run_evaluation_async(
    ...
    print("=" * 80)
    print("ScienceQA Evaluation (MAX ASYNC)")
    print("=" * 80)
    ...
    if not model_suffix:
        model_suffix = "model"
    output_file = os.path.join(output_dir, f"scienceqa_{mode_suffix}_{model_suffix}_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="ScienceQA Max Async Parallel Evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # CoT baseline only (no RAG)
  python evaluate_scienceqa.py --modes cot --max-questions 1273

  # RAG modes only
  python evaluate_scienceqa.py --modes direct baseline planning --max-questions 1273

  # All modes including CoT
  python evaluate_scienceqa.py --modes cot direct baseline planning --max-questions 100

  # Quick test (10 questions)
  python evaluate_scienceqa.py --max-questions 10

  # Full RAG evaluation with max parallelism
  python evaluate_scienceqa.py --max-questions 1273 --max-concurrent 100
        """
    )
