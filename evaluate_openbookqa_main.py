#!/usr/bin/env python3
"""
OpenBookQA Main Evaluation for RAG Comparison Experiment (Async Parallel Version)

This script evaluates different RAG modes on OpenBookQA Main testset
...
# ============================================================================
# OpenBookQA Main Dataset Loader
# ============================================================================
class OpenBookQAMainDataset:
    """OpenBookQA Main Dataset loader"""
    
    def __init__(self, benchmark_path: Optional[str] = None):
        if benchmark_path is None:
            benchmark_path = "/root/HypothesisRAG/data/openbookqa_main_train.json"
        ...
        self.dataset = benchmark["openbookqa_main_train"]
        self.index = sorted(self.dataset.keys())
        print(f"✓ Loaded {len(self)} OpenBookQA Main questions")
...
class AsyncRAGEvaluator:
    ...
    def generate_queries_grpo_rewriter(
        ...
        import torch
        from data.openbookqa_main_loader import format_rewriter_prompt
...
async def run_evaluation_async(
    ...
    print("=" * 80)
    print("OpenBookQA Main Evaluation (MAX ASYNC)")
    print("=" * 80)
    ...
    if not model_suffix:
        model_suffix = "model"
    output_file = os.path.join(output_dir, f"openbookqa_main_{mode_suffix}_{model_suffix}_{timestamp}.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="OpenBookQA Main Max Async Parallel Evaluation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # CoT baseline only (no RAG)
  python evaluate_openbookqa_main.py --modes cot --max-questions 1273

  # RAG modes only
  python evaluate_openbookqa_main.py --modes direct baseline planning --max-questions 1273

  # All modes including CoT
  python evaluate_openbookqa_main.py --modes cot direct baseline planning --max-questions 100

  # Quick test (10 questions)
  python evaluate_openbookqa_main.py --max-questions 10

  # Full RAG evaluation with max parallelism
  python evaluate_openbookqa_main.py --max-questions 1273 --max-concurrent 100
        """
    )
